...
# stage 14: functions for separator check and warning message.
...
contains_separator() {
    local line="$1"
    # SEPARATOR is a global variable - some people recommend against them
    [[ "$line" == *${SEPARATOR}* ]] 
}
...
read line
contains_separator "$line" && warn_separator "$line"
